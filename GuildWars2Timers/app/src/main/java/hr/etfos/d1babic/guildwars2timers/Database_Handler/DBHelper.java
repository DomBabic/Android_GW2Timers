...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_EVENTS_TABLE = "CREATE TABLE " + TABLE_EVENTS + "(" + KEY_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + KEY_EVENT_TITLE + " TEXT," + KEY_ICON + " INTEGER," + KEY_LOCATION + " TEXT," + KEY_DESCRIPTION + " TEXT," + KEY_TIMER + " TEXT," + KEY_SHIFT + " TEXT," + KEY_SUBSCRIBED + " INTEGER" + ")";
    ...
    public ArrayList<WorldEvent> getEvents() {
        ...
        if (cursor.moveToFirst()) {
            do {
                WorldEvent event = new WorldEvent(cursor.getInt(0), cursor.getString(1), cursor.getInt(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getInt(7));
                events.add(event);
        ...
        if(cursor.moveToFirst()) {
            do {
                if(cursor.getInt(7) == 1) {
                    WorldEvent subItem = new WorldEvent(cursor.getInt(0), cursor.getString(1), cursor.getInt(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getInt(7));
